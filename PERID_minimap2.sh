...
/DATA/home/egonzalez/SOFTWARE/Minimap2/minimap2.28/minimap2-2.28_x64-linux/minimap2 -cx asm5 HIFI.patch.NANO.fasta $i >  "$name"".paf"


done

################################################
#step 2 create the table for the plot
###############################################
for i in $(ls *paf)
do
...
awk '$12 >= 30 {print $8, $9, $3, $4, ($10*100)/$11, $1, $6, $5 }' $i > "$name""MQ30_tab.txt"

done

#in the script above, we filter the alignments for a MQ >= 30 like this $12 >= 30 and we calculate %ID like this ($10*100)/$11 
#here it is the info about paf files: https://github.com/lh3/miniasm/blob/master/PAF.md

########################################################
#step 3  plot al the chromosomes in one plot with R, you could save the script form below in a R script with the name PLOT_cont2_allCHR_PER.R
########################################################
library(dplyr)
library(magrittr)
              ...
              # INV <- read.table(bed2,  header=F)
               ...
               #INV_R <- INV$V8
                ...
                # name2 <-  paste(name, chr, "Ha412", ".jpg" , sep="_")
                 ...
                 if(h == "-") c(d,c) else c(c,d) }, mumgp$rs, mumgp$re, mumgp$qs, mumgp$qe, mumgp$sim, mumgp$qid, mumgp$rid, mumgp$stra
nd))
                 ...
                 #mumgp[c("qs", "qe")] <- t(mapply(\(a, b, c, d, e, f, g, h){
                  #  if(h == "-") c(d,c) else c(c,d) }, mumgp$rs, mumgp$re, mumgp$qs, mumgp$qe, mumgp$error, mumgp$qid, mumgp$rid, mumg
p$strand))
 ...
 #                mumgp <- subset(mumgp, mumgp$rid == chr) #& mumgp$rs > st & mumgp$re < end )
        ...
        #add vertical lines to the transposition region
            #  data_vline <- data.frame(group = c("Chr2", "Chr3", "Chr4", "Chr5", "Chr6", "Chr6", "Chr6", "Chr6", "Chr8", "Chr9", "Chr1
1", "Chr12", "Chr14", "Chr15", "Chr16", "Chr17"), vline
#=c(NA, NA, NA, NA, 125000000, 130000000, 130000000, 155000000, NA, NA, NA, NA, NA, NA, NA, NA))
  ...
  xlab('reference sequence') + ylab('assembly') +  scale_colour_gradient2(low = "blue", mid = "green", high = "red", midpoint = 97.5) #
+I also chenged this for anchorwave
...
jpeg( paste(name, "Sim_minNULL_mid65_ANCH.jpg", sep ="_"),  width=4000, height= 4000)
print(P1)
dev.off()

#the way to control for the colour the plots is in the line 158 above: scale_colour_gradient2(low = "blue", mid = "green", high = "red", midpoint = 97.5) 
# the midpoint is going to look green, more that the mindpoint is going to look red and less than the midpoint is going ot look blue

#########################################################
#step 4 run the script from above
#########################################################
for j in $( ls *_tab.txt)
...
###############################################################
#if you want to have one plot per chromosome, you could name this script like PLOT_cont2_PER_CHR.R
###############################################################
library(dplyr)
library(magrittr)
library(GenomicRanges)
...
jpeg( paste(name, chr, "Sim_minNULL_mid65_ANCH.jpg", sep ="_"),  width=4000, height= 4000)
print(P1)
dev.off()

#####################################################################
#to run the script from above
#####################################################################
...
do
   ...
   for i in $(cut -d " " -f 7 $j |sort |  uniq | head -n 17)
   do


   Rscript PLOT_cont2_PER_CHR.R $j $i
